...
class SpectralBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.view(x.size(0), -1)  # 输出 (B, 128)
...
# 双分支神经网络（不使用Attention和Transformer）
class DBNN_multinn(nn.Module):
    ...
    def forward(self, spectral_input, spatial_input):
        ...
        fused_feat = spectral_feat + spatial_feat             # 简单相加融合 (B, 128)
        output = self.classifier(fused_feat)                  # 输出 (B, num_classes)
        return output
